...
import { useAllWidgets, Widget, useBundleSocial, toggleFollow } from '@/hooks/useFirestore';
import BundleIframe from '@/app/components/BundleIframe';

export default function ExploreReelsPage() {
  const { user } = useAuth();
  const { widgets, loading } = useAllWidgets({ limitCount: 200 });
  const [index, setIndex] = useState(0);
  const list = useMemo(() => (widgets.length ? widgets : []), [widgets]);
  ...
  const social = useBundleSocial(current?.id, user?.uid, 'widget');
        ...
        <div style={{ width: '100%', maxWidth: 900 }}>
          <div className="explore-widget-card" style={{ padding: 0 }}>
            <div className="explore-widget-header" style={{ padding: '16px' }}>
              ...
              <div className="explore-widget-user">by {current.userId?.slice(0, 6) || 'Unknown'}</div>
            </div>
